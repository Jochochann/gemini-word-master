  ...
  return (
    <div className="relative w-full max-w-xl mx-auto px-1 sm:px-0 flex flex-col items-center">
      <div className="w-full h-[480px] sm:h-[620px] bg-white rounded-3xl shadow-xl border border-slate-200 flex flex-col p-7 sm:p-12 relative overflow-hidden">
            ...
            {item.example && (
              <div className="bg-slate-50 p-5 sm:p-7 rounded-2xl border border-slate-100 relative group">
                <div className="flex justify-between items-start mb-2">
                  ...
                  {lang !== 'zh-TW' && (
                    <button onClick={handleSpeakExample} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-white rounded-xl transition-all">
                      <Volume2 size={20} />
                    </button>
                  )}
                </div>
        ...
        <button disabled={isFirst} onClick={(e: React.MouseEvent) => { e.stopPropagation(); onPrev(); }} className={`hidden sm:flex absolute -left-16 top-1/2 -translate-y-1/2 p-3.5 rounded-full ${isFirst ? 'text-slate-200' : 'bg-white shadow-lg text-slate-600'}`}>
          <ChevronLeft size={30} />
        </button>
        <button disabled={isLast} onClick={(e: React.MouseEvent) => { e.stopPropagation(); onNext(); }} className={`hidden sm:flex absolute -right-16 top-1/2 -translate-y-1/2 p-3.5 rounded-full ${isLast ? 'text-slate-200' : 'bg-white shadow-lg text-slate-600'}`}>
          <ChevronRight size={30} />
        </button>
      </div>
      <div className="mt-8 flex sm:hidden justify-center items-center space-x-8">
        <button disabled={isFirst} onClick={onPrev} className={`p-5 rounded-full ${isFirst ? 'bg-slate-200 text-slate-400' : 'bg-white text-slate-700 shadow-lg'}`}><ChevronLeft size={28} /></button>
        <div className="px-6 py-3 bg-slate-800 text-white rounded-full font-bold text-sm tracking-widest shadow-inner">{item.id}</div>
        <button disabled={isLast} onClick={onNext} className={`p-5 rounded-full ${isLast ? 'bg-slate-200 text-slate-400' : 'bg-blue-600 text-white shadow-lg'}`}><ChevronRight size={28} /></button>
      </div>
      <style>{`.custom-card-scrollbar::-webkit-scrollbar { width: 4px; } .custom-card-scrollbar::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; }`}</style>
    </div>
  );
};
